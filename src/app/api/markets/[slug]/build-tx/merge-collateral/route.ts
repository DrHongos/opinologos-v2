...
import { buildMergeCollateralTxs, parseUsdcToWei, getChainId } from '@/lib/tx-builder';
...
export async function POST(
  ...
  return Response.json({
    transactions: buildMergeCollateralTxs(rows[0].os_index as `0x${string}`, amountWei),
    chainId: getChainId(),
    note: 'Burns equal amounts of all outcome tokens to recover collateral.',
  });
}
