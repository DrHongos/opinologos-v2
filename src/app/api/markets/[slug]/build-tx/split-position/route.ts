...
import { buildSplitPositionTxs, parseUsdcToWei, getChainId } from '@/lib/tx-builder';
...
export async function POST(
  ...
  return Response.json({
    transactions: buildSplitPositionTxs(rows[0].os_index as `0x${string}`, parentLinearIdx, condition as `0x${string}`, amountWei),
    chainId: getChainId(),
    note: 'Splits parent outcome tokens into leaf outcomes of the given condition.',
  });
}
